...
<head>
    <title>Activar Punto Turístico</title>
...
<body>
    <div id="popup" class="overlay">
        <div id="popupBody">
            <h2>Activar Punto Turístico</h2>
            <a id="cerrar" href="PuntosAdmin.php">&times;</a>
            <div class="popupContent">
                <?php
                require_once "../ConeccionBD/conexion.php";
                $db = Db::conectar();
                $id = $_GET['id'];
                $select = $db->prepare('SELECT * from puntoturistico WHERE id = ' . $id . '');
                $select->execute();
                while ($row = $select->fetch(PDO::FETCH_ASSOC)) {
                ?>
                    <form class="" action="../Controladores/controladorPuntoTuristico.php" method="post">

                        <div class="form-group ">
                            <input class="form-control" type="text" name="id" value="<?= $row['id'] ?>" style="display: none">
                        </div>
                        <div class="form-group ">
                            <label>De verdad deseea volver a Activar el Punto Turístico "<?php
                                                                                $nombre = $row['nombre'];
                                                                                echo ($nombre); ?>"</label>
                        </div>
            </div>
            <p>
                ...
                <button class=""><a href="PuntosAdmin.php">Cancelar</a></button>
                </form>
            <?php  } ?>
        </div>
    </div>
