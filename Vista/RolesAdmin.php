      ...
      <td style="margin-right: 0px;">
        <div class="header">
          <div class="header-right">
            <a class="active" href="RolesAdmin.php">Roles</a>
...
<body>
    <div class="tablaborde">
        <div class="col-md-12">
            ...
            <a href="crearRol.php" class="btn btn-success pull-right">Agregar nuevo rol</a>
        </div>
        <?php
        require_once "../ConeccionBD/conexion.php";
        $sql = "SELECT * FROM rol ORDER BY estado DESC";
        $pdo = Db::conectar();
        if ($result = $pdo->query($sql)) {
            if ($result->rowCount() > 0) {
                ...
                while ($row = $result->fetch()) {
                    ...
                    echo "<td>";
                    if ($row['estado'] == 1) {
                        echo "<a href='actualizarRol.php?id=" . $row['id'] . "' title='Actualizar Rol' data-toggle='tooltip'><span class='glyphicon glyphicon-pencil'></span></a>  &nbsp;";
                        echo "<a href='eliminarRol.php?id=" . $row['id'] . "' title='Eliminar Rol Record' data-toggle='tooltip'><span class='glyphicon glyphicon-remove'></span></a>  &nbsp;";
                    } else {
                        echo "<a href='activarRol.php?id=" . $row['id'] . "' title='Activar Rol' data-toggle='tooltip'><span class='glyphicon glyphicon-ok'></span></a>";
                    }
                    echo "</tr>";
                }
                echo "</tbody>";
