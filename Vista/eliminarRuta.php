...
<body>
    <div id="popup" class="overlay">
        <div id="popupBody">
            <h2>Eliminar Ruta</h2>
            <a id="cerrar" href="rutasTurista.php">&times;</a>
            <div class="popupContent">
                <?php
                require_once "../ConeccionBD/conexion.php";
                $db = Db::conectar();
                $id = $_GET['id'];
                $select = $db->prepare('SELECT * from ruta WHERE id = ' . $id . '');
                $select->execute();
                ...
                ?>
                    <form class="" action="../Controladores/controladorRuta.php" method="post">
                        ...
                        <div class="form-group ">
                            <label>De verdad deseea elimar su Ruta "<?php
                                                                            $nombre = $row['nombre'];
                                                                            echo ($nombre); ?>"</label>
                        </div>
            </div>
            ...
            <p>
                ...
                <button class=""><a href="rutasTurista.php">Cancelar</a></button>
                </form>
            <?php  } ?>
        </div>
    </div>
